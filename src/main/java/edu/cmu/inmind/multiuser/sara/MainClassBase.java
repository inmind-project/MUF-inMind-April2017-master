...
/**
 * Created by oscarr on 3/20/17. Main class for MUF 2.8+
 */
public class MainClassBase {
	...
	public void execute(List<ShutdownHook> hooks) throws Throwable {
		...
		muf = MUFLifetimeManager.startFramework(createModules(), createConfig(), null);
		if (hooks != null) {
	...
	protected Config createConfig() {
		return new Config.Builder()
				...
				.setDefaultNumOfPoolInstances(10).setNumOfSockets(1)
				// or you can refer to values in your config.properties file:
				.setPathLogs(Utils.getProperty("pathLogs")).setCorePoolSize(1000).setSessionTimeout(5, TimeUnit.DAYS) // dirty
																														// workaround
																														// for
																														// broken
																														// close-session
				.setServerAddress("tcp://128.237.134.168") // ALWAYS check to use IP and NOT 'localhost' or 127.0.0.1
				.setExceptionTraceLevel(Constants.SHOW_ALL_EXCEPTIONS).setExceptionLogger(getExceptionLogger())// MUF
																												// Exceptions/NON_MUF
																												// Exceptions
				.build();
	}
