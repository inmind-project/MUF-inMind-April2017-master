/* -------------------------------------------------------------------------
   ...
   PantomineCompilter.java
     - A compiler for Pantomine in the BEAT gesture toolkit
...
public class PantomimeCompiler extends Compiler {
    /** Time in seconds between animator issuing speech command and sound starting. */
    private  double TTS_DELAY=0.7; //was 0.1: increase if sound is late
    ...
    public PantomimeCompiler(String host,int port) throws Exception {
	super();
  ...
  private final String[] READY_SHAPES={"arm_ready0","arm_ready1","arm_ready2","arm_ready3"};
	...
	if(type.equals("BEAT")) {
	   ...
	   */
	    ...
	    outputScript(0.0,"schedulerpoint :acttime "+time+" :object \""+source.getAttribute("TARGET")+"\"",out);
  ...
  public class PostureControl {
	...
	*/
    ...
    private String[][] UPS_NAMES={
      ...
      {"hand_ps_drop_hand", "arm_ps_drop_hand","hand_ps_drop_hand","arm_ps_drop_hand"}, //"drop_hand",
