/* -------------------------------------------------------------------------

   NVBPriorityFilter.java
     - An NVB filter for the BEAT gesture toolkit

   BEAT is Copyright(C) 2000-2001 by the MIT Media Laboratory.  
   All Rights Reserved.

   Developed by Hannes Vilhjalmsson, Timothy Bickmore, Yang Gao and Justine 
   Cassell at the Media Laboratory, MIT, Cambridge, Massachusetts, with 
   support from France Telecom, AT&T and the other generous sponsors of the 
   MIT Media Lab.

   For use by academic research labs, only with prior approval of Professor
   Justine Cassell, MIT Media Lab.

   This distribution is approved by Walter Bender, Director of the Media
   Laboratory, MIT.

   Permission to use, copy, or modify this software for educational and 
   research purposes only and without fee is hereby granted, provided  
   that this copyright notice and the original authors' names appear on all 
   copies and supporting documentation. If individual files are separated 
   from this distribution directory structure, this copyright notice must be 
   included. For any other uses of this software in original or modified form, 
   including but not limited to distribution in whole or in part, specific 
   prior permission must be obtained from MIT.  These programs shall not be 
   used, rewritten, or adapted as the basis of a commercial software or 
   hardware product without first obtaining appropriate licenses from MIT. 
   MIT makes no representation about the suitability of this software for 
   any purpose. It is provided "as is" without express or implied warranty.

   ------------------------------------------------------------------------*/

package beat.filter;

import java.util.*;

import org.w3c.dom.*;

import beat.utilities.XMLWrapper;

/** A nonverbal behavior filter which removes proposed behaviors whose priorities
    fall below a specified threshold. Note that behaviors which do not define a
    priority default to priority zero. For instance, the following tag
    <pre>
    &lt;GAZE DIRECTION="TOWARDS_HEARER" PRIORITY="5"&gt;

    </pre>
    would be filtered out if the priority threshold was set above 5.

    <h3>Change log:</h3>
    <table border="1">
    <tr><th>Date<th>Who<th>What</tr>
    <tr><td>4/1/01<td>T. Bickmore<td> Created. </tr>
    </table>
*/

public class NVBPriorityFilter extends NVBFilter {
    /** The default priority, used when none is specified. */
  public static final int DEFAULT_PRIORITY=0;
    /** The current priority threshold. */
  protected int priorityThreshold;
    /** Constructor requires an initial value for the priority threshold. */
  public NVBPriorityFilter(int thresh) {
    priorityThreshold=thresh;
  }
    /** Changes the current priority threshold. */
  public void setPriorityThreshold(int thresh) {
    priorityThreshold=thresh;
  }
    /** Does the work of the filter, an XML transducer. */
  public Document run(Document tree) {
    Vector nodes=XMLWrapper.getAllNodesOfType(tree,new Vector(),module.getNVBTypes().getNVBTypes());
    for(int i=0;i<nodes.size();i++) {
      Element node=(Element)nodes.elementAt(i);
      int existingPriority=getPriority(node);
      if(existingPriority<priorityThreshold)
	XMLWrapper.prune(node);
    };
    return tree;
  }

    /** Returns the priority for a nonverbal behavior element. */
  public static int getPriority(Element node) {
    String priorityString=XMLWrapper.getXMLAttribute(node,"PRIORITY");
    if(priorityString!=null && priorityString.trim().length()>0) 
      return Integer.parseInt(priorityString);    
    else
      return DEFAULT_PRIORITY;
  }
}
