...
/** Removes all gestures of type "beat", such as 
    ...
    from within the temporal duration of a Deictic gesture, such as
    <pre>
    &lt;GESTURE_BOTH&gt;
    &nbsp;&nbsp;...
    &nbsp;&nbsp;&lt;ACTION&gt;
...
public class DeicticBeatFilter extends NVBFilter {
    ...
    */
	...
	Vector deictics = xmlw.getAllNodesOfType(new String[]{"GESTURE_RIGHT" , "GESTURE_LEFT" , "GESTURE_BOTH"});

	for(int i = 0; i < deictics.size(); ){
	    Element e = (Element)deictics.get(i);
	    if(!(XMLWrapper.getXMLAttribute(e, "TYPE").equals("DEICTIC"))){
		deictics.remove(i);
	    }
	    else{
		i++;
	...
	Element deicticNode = null;
	int startIndex;
	int endIndex;

	for(int i = 0; i < deictics.size(); i++){
	    deicticNode = (Element)deictics.get(i);
	    startIndex = ((Integer)xmlw.getNXMLAttribute(deicticNode, "WORDINDEX")).intValue();
	    endIndex = startIndex + ((Integer)xmlw.getNXMLAttribute(deicticNode, "WORDCOUNT")).intValue();
