<!--  
   ...
   MetaGenerator.xsl
     - XSL-based generator for the BEATSimple gesture toolkit

   BEATSimple is Copyright(C) 2000-2001 by the MIT Media Laboratory.
   All Rights Reserved.
...
<!-- Copy information from UTTERANCE into a special META tag at the root of the tree. --> 

<xsl:variable name="speaker" select="UTTERANCE/attribute::SPEAKER"/>
<xsl:variable name="hearer" select="UTTERANCE/attribute::HEARER"/>

<xsl:template match="/" priority="10">
<xsl:choose>
<xsl:when test="$speaker and $hearer">
   <META SPEAKER="{$speaker}" HEARER="{$hearer}">
   <xsl:copy>
